...
def largeRectangle(frame): #finds the largest rectangle created in a frame by a black object
    mask=cv2.inRange(frame, blackLower,blackUpper) #creates mask of all the pixels
...
while(True):
    ...
    minx, maxx, miny, maxy, output=largeRectangle(frame)
    if count<stableTime:
        minxsum+=minx
        minysum+=miny
        maxxsum+=maxx
